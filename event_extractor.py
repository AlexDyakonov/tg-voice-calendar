...
class EventExtractor:
    ...
    def __init__(self):
        ...
        self.time_patterns = [
            ...
            r'в\s+(\d{1,2})\s+час',     # в 17 часов
            r'(\d{1,2})\.(\d{2})',      # 19.00
            r'(\d{1,2}):(\d{2})',       # 17:00
            r'(\d{1,2})\s+час',         # 17 часов
        ]
        ...
        self.event_keywords = [
            'встреча', 'собрание', 'конференция', 'звонок', 'созвон',
            'презентация', 'доклад', 'лекция', 'семинар', 'тренинг',
            'интервью', 'собеседование', 'дедлайн', 'сдача', 'экзамен',
            'день рождения', 'праздник', 'мероприятие', 'событие'
        ]
    ...
    def extract_event(self, text: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"Извлечено - время: {event_time}, дата: {event_date}, описание: {event_description}")
            ...
            if event_datetime < datetime.now():
                if self._has_weekday(text):
                    ...
                    logger.info(f"Событие в прошлом, перенесено на следующую неделю: {event_datetime}")
                elif 'завтра' not in text and 'сегодня' not in text:
                    # Переносим только если не указано явно "завтра" или "сегодня"
                    event_datetime += timedelta(days=1)
                    logger.info(f"Событие в прошлом, перенесено на завтра: {event_datetime}")
                else:
                    logger.info(f"Событие запланировано на указанную дату: {event_datetime}")
                    ...
                    if len(match.groups()) == 3:  # ДД.ММ.ГГГГ
                        ...
                        if match.group(2).isdigit():  # ДД.ММ
                            ...
                            if month <= 12 and day <= 31:
                                ...
                                logger.info(f"Найдена дата: {day:02d}.{month:02d}.{year}")
                                return datetime(year, month, day).date()
                        else:  # ДД месяц
    ...
    def _extract_description(self, text: str) -> Optional[str]:
        ...
        main_words = []
        if all([self.segmenter, self.morph_tagger, self.syntax_parser, self.ner_tagger]):
            try:
                doc = Doc(text)
    ...
    def _clean_description(self, text: str) -> str:
        ...
        try:
            ...
            stop_words = ['в', 'на', 'во', 'с', 'у', 'к', 'от', 'до', 'за', 'под', 'над', 'при', 'о', 'об']
            words = text.split()
